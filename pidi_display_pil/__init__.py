...
#        words = text.split(" ")
#        words = text.split()
        if(len(text.encode()) == len(text) and (text.count(" ") > 1)) or \
          (not len(text.encode()) == len(text) and ((len(text) < 12) or \
          (text.count(" ") > 3) or (len(text) < 24 and (" " in text)))):
            words = text.split()
            ...
            text2 = text.replace('<',' <').replace('>','> ').replace('＜',' <').replace('＞','> ').replace('｢',' ｢').replace('｣','｣ ').replace('「',' ｢').replace('」','｣ ')            
            words = split('[ （）／　()/]', text2)
            if(len(words) < 3) and (len(text2) > 23):
                if(max_lines > 1 and len(text2) > 20):
                    char_len = round(len(text2) / 3)
                    words = [text2[:char_len],text2[char_len:char_len * 2],text2[char_len * 2:]]
                    max_lines = 3
                elif(len(text2) > 10):
                    char_len = round(len(text2) / 2)
                    words = [text2[:char_len],text2[char_len:]]
                    max_lines = 2    
...
#        from fonts.ttf import EPMGOBLD2 as UserFont
#        from fonts.ttf import Jiyucho as UserFont
#        from fonts.ttf import nicomojiplus as UserFont
        from fonts.ttf import roundedmplus1cmedium as UserFont
...
#            new = new.convert().filter(ImageFilter.GaussianBlur(radius=5*self._downscale))
#        new = new.convert('RGBA').filter(ImageFilter.GaussianBlur(radius=1*self._downscale))
...
#        text_in_rect(self._text_draw, self._title, self._font, (margin, 95 * self._downscale, width - margin, 170 * self._downscale))
        text_in_rect(self._text_draw, self._title, self._font, (margin, 80 * self._downscale, width - margin, 180 * self._downscale))
